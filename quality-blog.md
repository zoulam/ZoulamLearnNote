---
description: 优质博客、文章以及学习资源。(以前端为主)
---

# 优质博客推荐

## 异军突起的webAssembly

[Blazor](https://dotnet.microsoft.com/apps/aspnet/web-apps/blazor)

>  这是一个基于c#开发的webui库

[blazor入门安装](https://www.cnblogs.com/stulzq/p/12984971.html)

## **前端生态**

### 视图框架

>  [三大ui框架相关问答](https://www.zhihu.com/search?q=angular%20vue%20react&utm_content=search_suggestion&type=content)
>
> [Angular、React、Vue，谁能成为2021年JavaScript最佳框架？](https://zhuanlan.zhihu.com/p/210684223)

[angular](https://github.com/angular/angular)

[vue](https://github.com/vuejs/vue)

> 脚手架
>
> [vue-router](https://github.com/vuejs/vue-cli)
>
> 状态管理器
>
> [vuex](https://github.com/vuejs/vuex)
>
> 路由
>
> [vue-router](https://github.com/vuejs/vue-router)
>
> 原生/小程序
>
> [mpvue](https://github.com/Meituan-Dianping/mpvue)
>
> [taro](https://github.com/NervJS/taro) 没用过，不过看起来超强
>
> 插件
>
> 

[react](https://github.com/facebook/react)

> ​	`react`
>
> 脚手架
>
> [create-react-app](https://github.com/facebook/create-react-app) 简称cra
>
> 状态管理器
>
> [mobx](https://github.com/mobxjs/mobx)
>
> [redux](https://github.com/reduxjs/redux)              
>
> ​	[react-redux](https://github.com/reduxjs/react-redux)				
>
> ​		[redux-thunk](https://github.com/reduxjs/redux-thunk) 异步
>
> ​		[redux-saga](https://github.com/redux-saga/redux-saga) 异步
>
> ​		[redux-logger](https://github.com/LogRocket/redux-logger) 日志
>
> 路由
>
> [react-router](https://github.com/ReactTraining/react-router)
>
> 二次封装的库
>
> [umi](https://github.com/umijs/umi) umi是基于react封装的库，
>
> 原生/小程序
>
> [react-native](https://github.com/facebook/react-native)

### UI库

[vant](https://github.com/youzan/vant) 

[ant-design](https://github.com/ant-design/ant-design)

[element](https://github.com/ElemeFE/element)

[material](https://github.com/angular/material)

[bootstrap](https://www.bootcss.com/)

### css预处理器

[less](http://lesscss.org/)

[sass](https://sass-lang.com/guide)

​	[dart-sass](https://sass-lang.com/dart-sass) 官方主推的编译工具，比起node-sass编译速度更快，同时官方也说新特新会先在dart-sass上线。

[stylus](https://stylus.bootcss.com/)

### 测试框架

[jest](https://github.com/facebook/jest)

### 打包工具

[webpack](https://github.com/webpack/webpack)

​		[babel](https://github.com/babel/babel) js语法转化器，基于[core-js](https://github.com/zloirock/core-js) 这个polyfill库开发的JavaScript编译器。

[vite](https://github.com/vitejs/vite)

[rollup](https://github.com/rollup/rollup)

[gulp](https://github.com/rollup/rollup)

[Browserify](https://github.com/browserify/browserify)

### 网络请求库

[axios](https://github.com/axios/axios)

新的api虽然不怎么新了 [fetch](https://developer.mozilla.org/zh-CN/docs/Web/API/Fetch_API)

### 服务端框架

[nest.js](https://github.com/nestjs/nest)

[express](https://github.com/expressjs/express)

[koa](https://github.com/koajs/koa)

[midway](https://github.com/midwayjs/midway)

[eggjs](https://github.com/eggjs/egg)

### **很像的三个东西**

[nextjs](https://github.com/vercel/next.js) react服务端渲染框架

[nest.js](https://github.com/nestjs/nest) 类angular语法的ioc后端框架

[nuxt.js](https://www.nuxtjs.cn/) nuxt.js vue服务端渲染框架

### **提升js编码能力的库**

[google closure library](https://github.com/google/closure-library)

[fbjs](https://github.com/facebook/fbjs)

[lodash](https://github.com/lodash/lodash) 引入惰性计算，性能是underscore的数倍

[backbone](https://github.com/jashkenas/backbone) 中文意思是脊柱

[underscore](https://github.com/jashkenas/underscore) 中文意思是强调

​		[由 Underscore 与 Lodash 的差异引发的思考](https://cloud.tencent.com/developer/article/1385328)

​		[如何百倍加速 Lo-Dash？引入惰性计算](https://link.zhihu.com/?target=http%3A//justjavac.com/javascript/2015/12/15/introducing-lazy-evaluation.html)

### other

[typescript](https://www.typescriptlang.org/)

[node](https://github.com/nodejs/node) 万恶之源，基于v8引擎使用c++开发的JavaScript语法的服务端语言，即让JavaScript脱离浏览器的开端。

[threejs](https://github.com/mrdoob/three.js) web3d库

[jquery](https://github.com/jquery/jquery)

[animation.css](https://github.com/animate-css/animate.css) css动画库

[velocity](https://github.com/julianshapiro/velocity) js动画库 velocity的中文意思是速度/快速的意思

[electron](https://github.com/electron/electron)

[flutter](https://github.com/flutter/flutter) i just want to die,base dart programming language,build low performance native app framework.

[eslint](https://github.com/eslint/eslint)代码风格检查

[tslint](https://github.com/palantir/tslint)代码风格检查

[npm](https://yarnpkg.com/)

[yarn](https://www.npmjs.com/)

~~我发现写着写着跟高程的附录内容越来越像了~~

## 必看博客（属于不看就是亏钱的）

两个都是HcySunYang的博客

[渲染器](http://hcysun.me/vue-design/zh/essence-of-comp.html)

[Vue2.1.7源码学习](http://hcysun.me/2017/03/03/Vue%E6%BA%90%E7%A0%81%E5%AD%A6%E4%B9%A0/)

## 知名团队博客

[掘金翻译计划](https://github.com/xitu/gold-miner) 质量还不错，

[网易云音乐大前端团队](https://www.zhihu.com/column/musicfe)

[美团技术团队](https://tech.meituan.com/tags/前端.html)

[淘宝前端团队fed](https://fed.taobao.org/)

[百度前端团队](http://fex.baidu.com/)

[京东设计体验部-凹凸实验室](https://aotu.io/)

[腾讯alloyteam](http://www.alloyteam.com/)

[腾讯前端IMWEB团队](https://imweb.io/)

[腾讯前端IMWEB团队-github](http://imweb.github.io/)

[腾讯游戏](https://tgideas.qq.com/)

[~~大前端（饿了么）~~](https://www.daqianduan.com/)

[飞猪前端团队](https://www.zhihu.com/column/fliggy) 这个主要看周刊

[V8、Chrome、Node.js](https://www.zhihu.com/column/v8core) 这是一个专栏，里面我内容我一个都看不懂
			[Promise V8 源码分析(一)](https://zhuanlan.zhihu.com/p/264944183)

​			[Promise V8 源码分析(二)](https://zhuanlan.zhihu.com/p/329201628)

[若川的前端视野](https://www.zhihu.com/column/lxchuan12) 这人好像挺喜欢看源码，爱了爱了。

[nodejs源码分析](https://www.zhihu.com/column/c_1094251741922619392) 看着就头晕

[克莱登大学·计算机科学与技术](https://www.zhihu.com/column/c_173369107) 前端的，别被标题骗了，质量是入门级的。

## 仓库和视频

[CSS 奇技淫巧](https://github.com/chokcoco/iCSS)

[常用DEMO](https://github.com/zxuqian/html-css-examples) 配套的视频教程：[23个 HTML CSS JavaScript 特效合集](https://www.bilibili.com/video/BV1Rz4y1D7Lp?p=1)

[油管上的youtuberCSS教程](https://www.youtube.com/channel/UCbwXnUipZsLfUckBPsC7Jog) 扯个题外话，油管有挺多编程大神的，可惜前端方向的好像比较少。

[ant-design](https://github.com/ant-design/ant-design) 圣诞彩蛋！！！bad、bad、bad

[bootstrap](https://github.com/twbs/bootstrap)

[~~elementui~~](https://github.com/ElemeFE/element)不更新啦，后面又被人`fork`接盘了

[javascriptinfo](https://zh.javascript.info/):除了`MDN`没有比他更dio的了

## JavaScript

[阮一峰ES6](https://es6.ruanyifeng.com/)

[PicGo作者](https://molunerfinn.com/)

[nodemongoDB](http://mongodb.github.io/node-mongodb-native/)

[抽象语法树生成器](https://astexplorer.net/)

## 社区

[掘金](https://juejin.im/timeline) ：不过广告倒是挺少的，用户的阿里云广告倒是一大堆，以前看的时候还行，最近充满吊打面试官，万字水文等内容，看到好文章属于沙里淘金，所以建议是自行搜索，不看推荐。 

​		~~据说给字节跳动收购了？~~

[思否](https://segmentfault.com/)：还凑合。

## 优质文章

[14W 行代码量的前端页面长什么样](https://zhuanlan.zhihu.com/p/334617737) tql了！！！

[libuv源码简单分析](https://blog.butonly.com/posts/node.js/libuv/1-libuv-overview/)

[前端元编程——使用注解加速你的前端开发](https://zhuanlan.zhihu.com/p/274328551)

[CSS常见样式实现](https://juejin.im/post/5b1f41246fb9a01e725131fb#heading-24)

[CSS 排版与正常流 —— 重学CSS](https://zhuanlan.zhihu.com/p/265698290)

[Cli简单实现](https://www.cnblogs.com/buzhiqianduan/p/7655612.html)

[优质up主](https://space.bilibili.com/406004637?from=search&seid=13586037339355736354)但已经不更新了，里面有将一个 `chalk` 就是美化命令行的`node.js`库，是各大开源库脚手架的依赖之一

[css面试题](https://juejin.im/post/5ee0cf335188254ec9505381)

[2020不容错过！24个CSS新特性来了](https://zhuanlan.zhihu.com/p/267066908)我已经晕过去了。

[10种跨域解决方案](https://juejin.im/post/5e948bbbf265da47f2561705)

[CSS 排版与正常流 —— 重学CSS](https://zhuanlan.zhihu.com/p/265698290)

[移动端开发一些常见问题的解决方案](https://zhuanlan.zhihu.com/p/307355746)待实践

[我是如何从零开始学TypeScript的](https://zhuanlan.zhihu.com/p/240069181) 我觉得这个不算优质文章，😁

[【万字长文】深入理解 TypeScript 高级用法](https://zhuanlan.zhihu.com/p/136254808)

[详解Typescript里的This](https://zhuanlan.zhihu.com/p/104565681)

[如何基于 TypeScript 实现控制反转](https://zhuanlan.zhihu.com/p/311184005)暂时看不太明白

[几道前端笔试题](https://zhuanlan.zhihu.com/p/80366959) 挺恶心的2019-09-11的文章。

[10分钟教你手写8个常用的自定义hooks](https://zhuanlan.zhihu.com/p/298172332) 入门级文章

[携程是如何做React Native优化的](https://zhuanlan.zhihu.com/p/23715716) dio啊

[【译】Flutter vs React Native vs Native：深度性能比较](https://zhuanlan.zhihu.com/p/153132747)

[React Native 新架构](https://zhuanlan.zhihu.com/p/281238593)

[再也不怕面试官问你express和koa的区别了](https://zhuanlan.zhihu.com/p/87079561) 质量一般

[Jamstack，下一代Web建站技术栈？](https://zhuanlan.zhihu.com/p/281085404)mark

[可视化研发之线的画法：直线、曲线和动画（WebGL篇）](https://zhuanlan.zhihu.com/p/279303291)

## 知乎博主

鬼一样的地方，老娱乐圈了

[贺师俊](https://www.zhihu.com/people/he-shi-jun) 好像是做 ecma script规范的，听别人都叫他贺老想来年纪不小了。

[尤雨溪](https://www.zhihu.com/people/evanyou) 😛

## 解析JavaScript代码

[babel-try](https://babeljs.io/repl#?browsers=defaults%2C%20not%20ie%2011%2C%20not%20ie_mob%2011&build=&builtIns=false&spec=false&loose=false&code_lz=MYewdgzgLgBApgGzgWzmWBeGAeAFgRgD4AJRBEAGhgHcQAnBAEwEJsB6AwgbgChRJY_KAEMAlmDh0YWRiGABXVOgB0AczhQAokiVQAQgE8AkowAUAcjogQUcwEpeAJTjDgUACIB5ALLK6aRklTRBQ0KCohMQk6Bx4gA&debug=false&forceAllTransforms=false&shippedProposals=false&circleciRepo=&evaluate=false&fileSize=false&timeTravel=false&sourceType=module&lineWrap=true&presets=react&prettier=false&targets=&version=7.11.6&externalPlugins=)

[sandbox](https://codesandbox.io/) 在线编程环境用于学习好用

## 一些解答疑惑的知乎问题

[一年内的前端看不懂前端框架源码怎么办？](https://www.zhihu.com/question/350289336/answer/873350617)

- [ ] [新手上路学习JavaScript引擎实现——路线图](https://zhuanlan.zhihu.com/p/20505562) 看不懂

    ​			[lazyparser](https://space.bilibili.com/296494084?from=search&seid=10139030435981964205) 这个up主里面有v8引擎的教程，对于现在的我来说还很难。

    ​			[课程的仓库](https://github.com/lazyparser/v8-internals)

    [5分钟入门MP4文件格式](https://zhuanlan.zhihu.com/p/333765990)mark，说不定以后会用上
    
    [一个没有经验的前端工程师，写CSS的时候有什么常见通病？](https://www.zhihu.com/question/322443498/answer/669000990)
    

[前端搞那么多工具框架库，是让开发更简单，还是更复杂?](https://www.zhihu.com/question/56213172/answer/148117759) 里面[方应杭](https://www.zhihu.com/people/zhihusucks)的回答很通俗的解释了，让我也看懂了

[你靠哪些讲解学会了曾经怎么也学不会的算法？](https://www.zhihu.com/question/265531567/answer/295330414)

[ECMAScript的数字版本号和年份版本号是什么关系？](https://www.zhihu.com/question/61698736/answer/593286532) 虽然高程第四版有讲，但是还是这个好记。

[有哪些CSS标准是前端工程师很有必要研读的？](https://www.zhihu.com/question/41191048/answer/90058208) mark，暂时不管，先看mdn就好了。

[血泪教训之请不要再轻视Git —— 我在工作中是如何使用 Git 的](https://zhuanlan.zhihu.com/p/250493093)

[有哪些必看的 JS 库？](https://www.zhihu.com/question/429436558/answer/1572099805) 我就不太喜欢看

[前端如何突破技术与业务的瓶颈——Shopee一年半记](https://zhuanlan.zhihu.com/p/284525549)mark

## **八股文**

[60道前端经典高评率出现的面试题包含JS、CSS、React、网络、浏览器、程序题等（附答案）](https://zhuanlan.zhihu.com/p/267659458) 冲！！！



## Vue3

[" 源 " 来如此](https://www.zhihu.com/column/c_1162017153112363008) vue的狂热粉丝？？不过质量一般

[深入理解 Vue3 Reactivity API](https://zhuanlan.zhihu.com/p/146097763) dio

[问起Vue的原理时不要再只说defineProperty了](https://zhuanlan.zhihu.com/p/37131046) 哦豁

## React

[react入门](https://aotu.io/notes/2020/11/12/react-indoor/)

[图解React原理系列](http://www.7km.top/)

[React技术揭秘](https://react.iamkasong.com/diff/prepare.html)

[这可能是最通俗的 React Fiber(时间分片) 打开方式](https://juejin.cn/post/6844903975112671239#heading-6)

## other

[「薅」52图初探Linux通用知识](https://zhuanlan.zhihu.com/p/281865598)

[字节跳动面试必问的后台高性能、高并发设计指南开源啦!](https://zhuanlan.zhihu.com/p/322464806) 后端的

[计算机寻址能力与地址总线和数据总线的关系？](https://www.zhihu.com/question/311449926/answer/591509461)路过

[TCP/IP协议到底在讲什么？](https://www.zhihu.com/question/51074319/answer/124733136)

[作为程序员，你一般用什么软件画流程图时序图和状态图等？](https://www.zhihu.com/question/48780488/answer/1563335651)